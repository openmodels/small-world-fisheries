## Generate the file of network edges
...
#weights <- read.csv(paste0(datapath, "weights/combined-country-temp-nr.csv"))
groups <- read.csv(paste0(datapath, "saudata/species.csv"))

## EEZ to EEZ transitions
## Generated by transitions/analysis.R
transits <- read.csv(paste0(paste0(datapath, "spawn-transits/spawn-transits-rescaled", spawn.suffix, ".csv")))
...
outfile <- paste0(resultpath, "edgeweights",spawn.suffix,".csv")
write.csv(results,outfile,row.names=F)

write.csv(resspecies, paste0(resultpath, "edgecontribs", spawn.suffix, ".csv"), row.names=F)
